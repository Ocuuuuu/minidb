...
TEST_CASE("Constant recognition", "[lexer][constant]") {
    SECTION("Integer constants") {
        // 包含3个整数值
        Lexer lexer("123 0 456789");
        auto tokens = lexer.getAllTokens();

        // 预期：3个常量 + 1个EOF = 4个Token
        REQUIRE(tokens.size() == 4);
        REQUIRE(tokens[0].type == TokenType::CONSTANT);
        REQUIRE(tokens[0].value == "123");
        REQUIRE(tokens[1].value == "0");
    }

    // 测试浮点型常量
    SECTION("Float constants") {
        Lexer lexer("123.45 .67 89. 0.123");
        auto tokens = lexer.getAllTokens();
        std::cout << "Float tokens: " << tokensToString(tokens) << std::endl; // 调试输出
        REQUIRE(tokens.size() == 5); // 4��CONSTANT + 1��EOF
        REQUIRE(tokens[1].value == ".67"); // 验证.67被正确识别
    }
    ...
    SECTION("String constants with double quotes") {
        ...
        // 预期：3个字符串 + 1个EOF = 4个Token
        REQUIRE(tokens.size() == 4);
        REQUIRE(tokens[0].value == "hello");
        REQUIRE(tokens[1].value == "world 123");
        REQUIRE(tokens[2].value == "with\"quote");  // 注意转义引号
    }
